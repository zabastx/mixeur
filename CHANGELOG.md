...
- A rigged model — a character, anything posed by a skeleton — came back from a saved project or a JSON export crushed into a knot at the origin. Saving copied the model's bones but left its skeleton pointing at the originals, so the file named bones it did not contain and there was nothing to pose the model with when it was read back. Exporting to GLB broke the same way, writing a skin whose joints were all empty. Saving a project, exporting an object to JSON and exporting the scene to GLB all keep a rig whole now, however many meshes share it — the character this was found with has thirty on one skeleton of five hundred and fifty-nine bones
- A skinned mesh saved without the bones that pose it could only ever load broken, since the bones are what put it in shape. Exporting one on its own is now refused and says so, and saving a project or exporting a scene that has one names the model that will not survive the trip rather than writing it out quietly
